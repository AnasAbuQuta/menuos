...
function remove(item) { pendingDelete.value = item }
async function confirmRemove() {
  const item = pendingDelete.value
  if (!item || deleting.value) return
  deleting.value = true; pageError.value = ''
  try { await deleteMenuItem(item.id); items.value = items.value.filter(({ id }) => id !== item.id); toast.success('Menu item deleted successfully.'); pendingDelete.value = null }
...
async function move(item, offset) {
  ...
  items.value = reordered; reordering.value = true; pageError.value = ''
  const ids = [...group]; [ids[index], ids[target]] = [ids[target], ids[index]]
  try { await reorderMenuItems(item.category.id, ids.map(({ id }) => id)); toast.success('Menu item order saved.') }
...
<template>
  <section class="menu-items-page">
    ...
    <BaseLoading v-if="loading" :rows="5" label="Loading menu items" />
    <BaseEmptyState v-else-if="!pageError && items.length === 0" title="No menu items found" :message="categories.length ? 'Add an item or adjust your filters.' : 'Create a category first, then add your menu items.'"><button v-if="categories.length" class="button" type="button" @click="openCreate">Add first item</button></BaseEmptyState>
    ...
    <BaseConfirmDialog :open="Boolean(pendingDelete)" title="Delete menu item?" :message="`Delete “${pendingDelete?.name}”? This cannot be undone.`" confirm-label="Delete item" danger :loading="deleting" @confirm="confirmRemove" @cancel="pendingDelete = null" />
  </section>
</template>
