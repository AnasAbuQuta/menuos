...
function confirmHours(messageKey) { hoursFeedback.value = t(messageKey); toast.success(hoursFeedback.value) }
function makeTwentyFourHours(day) { setTwentyFourHours(form.opening_hours[day]); confirmHours('setup.hoursSet24') }
function copyPrevious(index) { copyHours(form.opening_hours[DAYS[index - 1]], form.opening_hours[DAYS[index]]); confirmHours('setup.hoursCopiedPrevious') }
function copyAll(day) { DAYS.forEach((target) => copyHours(form.opening_hours[day], form.opening_hours[target])); confirmHours('setup.hoursCopiedAll') }
...
async function submit() {
  ...
  try {
    ...
    try {
      ...
      toast.warning(t('setup.imagesUploadWarning'))
    }
...
<template>
  <section class="setup-page setup-wizard">
    ...
    <form class="card wizard-card" @submit.prevent="step === steps.length - 1 ? submit() : next()">
      ...
      <section v-else-if="step === 3"><h2>{{ t('setup.brandImages') }}</h2><p>{{ t('setup.brandImagesHelp') }}</p><div class="wizard-images"><label v-for="kind in ['logo', 'cover']" :key="kind" class="wizard-image" :class="`wizard-image-${kind}`"><span class="wizard-image-heading"><strong>{{ t(kind === 'logo' ? 'setup.logoRatio' : 'setup.coverRatio') }}</strong><small>{{ t(kind === 'logo' ? 'setup.logoHelp' : 'setup.coverHelp') }}</small></span><span class="wizard-image-preview"><img v-if="previews[kind]" :src="previews[kind]" :alt="t('setup.imagePreview', { type: t(`imageEditor.${kind}`) })"><span v-else class="wizard-image-empty">{{ t('setup.noImageSelected') }}</span></span><span class="button button-secondary wizard-image-picker">{{ t(previews[kind] ? 'setup.changeImage' : kind === 'logo' ? 'setup.chooseLogo' : 'setup.chooseCover') }}</span><input class="sr-only" type="file" accept="image/jpeg,image/png,image/webp" @change="chooseImage(kind, $event)"></label></div></section>
      ...
      <section v-else-if="step === 5"><h2>{{ t('setup.openingHours') }}</h2><p>{{ t('setup.openingHoursHelp') }}</p><p v-if="hoursFeedback" class="hours-feedback" role="status">✓ {{ hoursFeedback }}</p><div class="hours-list"><div v-for="(day, index) in DAYS" :key="day" class="hours-row"><strong>{{ t(`days.${day}`) }}</strong><label class="checkbox-label"><input v-model="form.opening_hours[day].is_open" type="checkbox" @change="toggleDay(day)">{{ t(form.opening_hours[day].is_open ? 'restaurant.open' : 'restaurant.closed') }}</label><template v-if="form.opening_hours[day].is_open"><div class="hours-times"><label><span>{{ t('setup.openingTime') }}</span><input v-model="form.opening_hours[day].open" type="time"></label><label><span>{{ t('setup.closingTime') }}</span><input v-model="form.opening_hours[day].close" type="time"></label></div><div class="hours-actions"><button type="button" class="text-button" @click="makeTwentyFourHours(day)">{{ t('restaurant.open24') }}</button><button v-if="index" type="button" class="text-button" @click="copyPrevious(index)">{{ t('restaurant.copyPrevious') }}</button><button type="button" class="text-button" @click="copyAll(day)">{{ t('restaurant.copyAll') }}</button></div></template></div></div></section>
