  ...
  <section class="restaurant-settings-page">
    ...
    <template v-else>
      ...
      <form class="settings-form" @submit.prevent="save">
        <section class="card settings-section"><div><h2>Basic Information</h2><p>Core restaurant identity and availability.</p></div><div class="settings-grid"><label>Restaurant name<input v-model="form.name" maxlength="255" :disabled="saving" required><span v-if="errorFor('name')" class="field-error">{{ errorFor('name') }}</span></label><label class="checkbox-label setting-toggle"><input v-model="form.is_active" type="checkbox" :disabled="saving">Restaurant is active</label><label class="full">Description<textarea v-model="form.description" rows="4" maxlength="5000" :disabled="saving" /><span v-if="errorFor('description')" class="field-error">{{ errorFor('description') }}</span></label></div></section>
