export const IMAGE_PROFILES = {
  logo: { aspectRatio: 1, ratios: [{ labelKey: 'square', value: 1 }], maxWidth: 800, maxHeight: 800 },
  cover: { aspectRatio: 16 / 9, ratios: [{ labelKey: '16:9', value: 16 / 9 }, { labelKey: '3:1', value: 3 }], maxWidth: 1920, maxHeight: 1080 },
  menuItem: { aspectRatio: 4 / 3, ratios: [{ labelKey: '4:3', value: 4 / 3 }, { labelKey: 'square', value: 1 }], maxWidth: 1200, maxHeight: 1200 },
}
...
export function validateImageFile(file) {
  if (!file || !SUPPORTED_IMAGE_TYPES.includes(file.type)) return 'imageType'
  if (file.size > MAX_IMAGE_BYTES) return 'imageSize'
  return ''
}
...
function canvasBlob(canvas, type, quality) {
  return new Promise((resolve, reject) => canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error('processError')), type, quality))
}
...
export async function processCroppedCanvas(sourceCanvas, sourceFile, profile) {
  ...
  if (blob.size > MAX_IMAGE_BYTES) throw new Error('processedTooLarge')
